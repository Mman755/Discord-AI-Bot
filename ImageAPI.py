...
def get_image(PROMPT):
    ...
    response = requests.post(
        ...
        headers=headers, # Specify headers which we made earlier
        json = data,
        timeout = 60 # Timeout was manually overridden to avoid premature timeouts as this call can take a while
    )
    ...
    if response.status_code == 200:
        ...
        return image_url
    # If the response code is not 200 then something must be wrong, tell user and return back an error message
    else:
